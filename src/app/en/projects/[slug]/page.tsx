import { ArrowLeft, ArrowRight, CalendarDays, Code2, ExternalLink, Layers3, Mail, Target, Zap } from "lucide-react";
...
export default async function EnglishProjectPage({ params }: Props) {
  ...
  return <main><div className="page-shell"><Header locale="en" /><article className="case-page">
    ...
    <section className="case-contact"><div><span>Have a similar challenge?</span><h2>Let&apos;s discuss your project</h2></div><div className="case-contact-actions"><a className="button light" href={profile.telegram} target="_blank" rel="noreferrer">Message on Telegram <ArrowRight size={18} /></a><a className="button contact-ghost" href={`mailto:${profile.email}`}>Email <Mail size={16} /></a><a className="button contact-ghost" href={profile.github} target="_blank" rel="noreferrer">GitHub <ExternalLink size={16} /></a><a className="button contact-ghost" href={profile.vk} target="_blank" rel="noreferrer">VK <ExternalLink size={16} /></a><a className="button contact-ghost" href={profile.instagram} target="_blank" rel="noreferrer">Instagram <ExternalLink size={16} /></a></div></section>
  </article></div></main>;
}
