...
export default async function EnglishProjectPage({ params }: Props) {
  ...
  return <main><div className="page-shell"><Header locale="en" /><article className="case-page">
    ...
    <section className={`case-hero${details.image ? "" : " case-hero-text"}`}><div className="case-hero-copy"><div className="case-eyebrow"><span>{details.status}</span><i />{details.period}</div><h1>{project.title}</h1><p>{project.summary}</p><div className="case-links">{project.links.map((link) => <a className="button secondary" href={link.href} target="_blank" rel="noreferrer" key={link.href}>{link.label} <ExternalLink size={17} /></a>)}</div></div>{details.image ? <PreviewFrame project={project} locale="en" /> : null}</section>
    ...
    <section className="case-story"><div className="case-story-title"><span>Concise breakdown</span><h2>Problem, implementation, outcome</h2></div><div className="case-story-grid"><article><Target size={23} /><span>01 / Problem</span><p>{details.challenge}</p></article><article><Layers3 size={23} /><span>02 / Implementation</span><p>{details.solution}</p></article><article><Zap size={23} /><span>03 / Outcome</span><p>{details.outcome}</p></article></div></section>
    <section className="case-section workflow-section"><div className="case-section-head"><span>Workflow</span><h2>What happens step by step</h2></div><WorkflowDiagram steps={project.workflow} locale="en" /></section>
    <section className="case-detail-grid"><div className="case-section"><div className="case-section-head"><span>Delivered</span><h2>Core features</h2></div><ul className="check-list">{project.highlights.map((item) => <li key={item}>{item}</li>)}</ul></div><div className="case-section stack-section"><div className="case-section-head"><span>Technology</span><h2>Stack</h2></div><div className="stack-list large">{project.stack.map((item) => <span key={item}>{item}</span>)}</div></div></section>
